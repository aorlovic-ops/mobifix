...
# =========================================================================
# 2. PORTAL ZA KLIJENTE (Prijava popravka)
# =========================================================================
...
def zaprimi_popravak(
    ...
    email: str = Form(...),
    brand: str = Form(...),
    model_uredaja: str = Form(...),  # Prihvaća polje iz HTML-a (model_uredaja)
...
# =========================================================================
# 3. PREUSMJERAVANJA ZA LOGIN I REGISTER
# =========================================================================
...
def uredi_nalog(
    ...
    opis_kvara: str = Form(...),
    status: str = Form(...),
    napomena_servisera: Optional[str] = Form(""),
    db: Session = Depends(get_db)
):
    nalog = db.query(ServisniNalog).filter(ServisniNalog.id == nalog_id, ServisniNalog.servis_id == ZADANI_SERVIS_ID).first()
    if nalog:
...
# --- DODANO: Endpoint za brisanje servisa iz Super Admin panela ---
@app.post("/super-admin/obrisi-servis/{servis_id}")
def obrisi_servis(servis_id: int, db: Session = Depends(get_db)):
    servis = db.query(Servis).filter(Servis.id == servis_id).first()
    if servis:
        # Obriši sve servise, klijente i pripadajuće naloge
        ...
        db.query(Servis).filter(Servis.id == servis_id).delete()
        db.commit()
